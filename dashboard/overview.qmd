...
def visualise_new_contributors_per_quarter(calculator: MetricsCalculator) -> None:
    ...
    if df.empty:
        ax.text(0.5, 0.5, 'No data available for this period',
                horizontalalignment='center', verticalalignment='center',
                transform=ax.transAxes, fontsize=12, color='red')
        ax.set_title('New Contributors per Quarter')
        plt.tight_layout()
        plt.show()
        return
    ...
    ax.bar(df['period'], df['new_contributors'], width=60,
...
def visualise_cumulative_contributors(calculator: MetricsCalculator) -> None:
    ...
    if df.empty:
        ax.text(0.5, 0.5, 'No data available for this period',
                horizontalalignment='center', verticalalignment='center',
                transform=ax.transAxes, fontsize=12, color='red')
        ax.set_title('Cumulative Contributors')
        plt.tight_layout()
        plt.show()
        return
    ...
    df = df.copy()
    df['period'] = pd.to_datetime(df['period'])
    ax.plot(df['period'], df['cumulative_contributors'],
            color=sns.color_palette('Set2')[0], linewidth=2)
    ax.fill_between(df['period'], df['cumulative_contributors'], 0,
                     color=sns.color_palette('Set2')[0], alpha=0.3)
    ax.xaxis.set_major_locator(mdates.YearLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
    ax.set_xlabel('Year')
...
```{python}
htmltools.HTML("<span class=\"card-title d-flex align-items-center\"> <i class=\"bi bi-graph-up-arrow\"> </i> <span class=\"ms-2\"> Cumulative contributors</span> </span>")
```
